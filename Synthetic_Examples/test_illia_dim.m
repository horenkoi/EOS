...
N_ann=20;
T=2000;
DD=[3 4 6 10 20 50 100 150 200 300 400 500 1000];% 250 300 400 600 800 1000];
EPS=[0.8 0.7 0.5 0.5 0.5 0.5 0.2 0.3 0.3 0.25 0.3 0.4 0.4];
...
figure;EB(DD,ACC_rob,'b-');hold on;EB(DD,ACC_rob2,'b:x');EB(DD,ACC_hadi,'gx:');EB(DD,ACC_for,'mx-');EB(DD,ACC_svm,'k<--');EB(DD,ACC_EOD,'ro--');set(gca,'XScale','log');
legend({'Olive-Hawkins Algorithm','Gnanadesikan-Kettenring Algorithm', 'Hadi Algorithm','Isolating Forest Algorithm', 'One-Class SVM', 'Entropic Outlier Detection (EOD)'},'FontSize',18,'Location','southwest')
         title({['Precision Comparison (T=' num2str(size(A,2)) ', p=' num2str(prop) ')']});set(gcf,'Position',[10 100 1300  700]);
         xlabel('Data Dimension, D');ylabel('Precision '); 
         set(gca,'FontSize',24,'LineWidth',2);box on;ylim([0 1.05]);xlim([2.5 1050])

figure;EB(DD,F1_rob,'b-');hold on;EB(DD,F1_rob2,'b:x');EB(DD,ACC_hadi,'gx:');EB(DD,F1_for,'mx-');EB(DD,F1_svm,'k<--');EB(DD,F1_EOD,'ro--');set(gca,'XScale','log');
legend({'Olive-Hawkins Algorithm','Gnanadesikan-Kettenring Algorithm', 'Hadi Algorithm','Isolating Forest Algorithm', 'One-Class SVM', 'Entropic Outlier Detection (EOD)'},'FontSize',18,'Location','southwest')
         title({['F1-score Comparison (T=' num2str(size(A,2)) ', p=' num2str(prop) ')']});set(gcf,'Position',[10 100 1300  700]);
         xlabel('Data Dimension, D');ylabel('F1-score '); 
         set(gca,'FontSize',24,'LineWidth',2);box on;ylim([0 1.05]);xlim([2.5 1050])

figure;EB(DD,t_rob,'b-');hold on;EB(DD,t_rob_2,'b:x');EB(DD,t_hadi,'gx:');EB(DD,t_for,'mx-');EB(DD,t_svm,'k<--');EB(DD,t,'ro--');set(gca,'XScale','log','YScale','log');
legend({'Olive-Hawkins Algorithm','Gnanadesikan-Kettenring Algorithm', 'Hadi Algorithm','Isolating Forest Algorithm', 'One-Class SVM', 'Entropic Outlier Detection (EOD)'},'FontSize',18,'Location','northwest')
         title({['Cost Comparison (T=' num2str(size(A,2)) ', p=' num2str(prop) ')']});set(gcf,'Position',[10 100 1300  700]);
         xlabel('Data Dimension, D');ylabel('CPU time, sec. '); 
         set(gca,'FontSize',24,'LineWidth',2);box on;xlim([2.5 1050])
