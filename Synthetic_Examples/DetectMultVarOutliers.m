function [mu,S,RD,RD_valid,chi_crt]=DetectMultVarOutliers(X,X_valid,n_out,id_in,vis)
...
elseif ~isnumeric(n_out) || numel(n_out)~=1 || n_out<2 || n_out>(N-d-1) || n_out~=round(n_out)
    error('Incorrect entry for 2nd input argument (k)')
end

if nargin<4 || isempty(id_in)
    id_in=false(N,1);
...
MD_valid=MahDistSquared(X_valid,mu,S);
MD_valid=sqrt(MD_valid);
%MD_srt_valid=sort(MD_valid,'ascend');
